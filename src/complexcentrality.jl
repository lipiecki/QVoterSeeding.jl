...
function complexcentrality(g::SimpleGraph{<:Integer}, T::AbstractVector{<:Integer}, v::Integer)
    ...
    while !diffused
        ...
        # add each newly activated vertex to the induced subgraph (exclude edges between newly activated vertices)
        for w in vertices(g)
            if activated_now[w]
                for neighbor in neighbors(g, w)
                    if active[neighbor]
        ...
        # calculate shortest complex path between the vertex `v` and each newly activated vertex
        for w in vertices(g)
            if activated_now[w]
                complex_paths_len += length(a_star(h, 1, keys[w])) + 1
            end
        end
        # update `active` vector and include edges between newly activated vertices
        for w in vertices(g)
            if activated_now[w]
                active[w] = true
                for neighbor in neighbors(g, w)
                    if activated_now[neighbor]
                        add_edge!(h, keys[w], keys[neighbor])
                    end
                end
            end
        end
        fill!(activated_now, false)
    end
...
Calculate the [complex centrality](https://doi.org/10.1038/s41467-021-24704-6) for graph `g` with threshold values adjusted to `q`-voter dynamics, according to: ``T_i = \\max\\{q, \\lceil k_i/2\\rceil\\}``, where ``k_i`` is the degree of vertex ``i``.
