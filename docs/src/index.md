...
- seeding nodes with the highest complex centrality (adapted to q-voter dynamics)
- seeding nodes with the highest degree centrality
- seeding nodes with the highest PageRank centrality
- seeding nodes using one-hop
- seeding random nodes
...
```
where `results` is a vector of final concentrations of active nodes obtained for each seeding budget.
